...
func NewSecurity(username, passwd string, timeOffset time.Duration) Security {
	...
	created := time.Now().UTC().Add(timeOffset).Format(time.RFC3339Nano)
	fmt.Println("REQUEST TIME!! ", created)
	auth := Security{
		Auth: wsAuth{
			Username: username,
			Password: password{
